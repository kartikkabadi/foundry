...
import { type IssueSize } from "@/lib/foundry/types";
...
export async function POST(request: Request) {
  const body = (await request.json()) as { idea?: string; targetUrl?: string; size?: string };
  ...
  const issue = createIssue({ idea, targetUrl, size });
