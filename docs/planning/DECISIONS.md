...
**Pi Composer Powerpack** ([kartikkabadi/pi-composer-powerpack](https://github.com/kartikkabadi/pi-composer-powerpack)): Guide-style Pi package + curated assets. Install via `pi install` from GitHub only — no `install.sh` or second npm setup path. Agent bootstrap: `docs/agents/README.md` in that repo. Canonical clone: `/Users/user/Projects/pi-composer-powerpack`.

**Foundry (this repo)**: Planning-first TypeScript/Node CLI (doctor, setup, plan, build with Composer 2.5). Agent bootstrap: `docs/agents/README.md`.

**Relationship**: Separate products — powerpack is the Pi extension/guide layer; Foundry is the planning/build runtime. Cross-link in READMEs; do not merge responsibilities.

**Local clones:** `/Users/user/Documents/projects/foundry` and `/Users/user/Projects/pi-composer-powerpack`.
